...
list_f = open(os.path.join(DATA_PATH, "sets/test.txt")).readlines()
for line in list_f:
    img_path = os.path.join(DATA_PATH, "images", line.replace('\n', '.jpg'))
    input_img = Image.open(img_path)
    input_img = toTensor(input_img).to(device)
    input_img = input_img.unsqueeze(0)

    img_path = os.path.join(DATA_PATH, "masks", line.replace('\n', '.png'))
    gt_img = Image.open(img_path)
    gt_img = np.array(gt_img)   # [H, W]
    gt_img[gt_img==255] = 0

    out_seg = unet(input_img)
    ...
    out_img.save(os.path.join("./segs", line.replace('\n', '.png')))
